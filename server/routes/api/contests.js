...
module.exports = (app) => {
    ...
    app.get('/api/contests/globalRankList', function(req, res) {
        User.find({
            ...
            for (let user of users) {
                ...
                pushObject = Object.assign(
                    {usn: user.usn, name},
                    user.contender.toObject());
    ...
    app.post('/api/contests/updateContenders',
        requireRole('admin'),
        function(req, res) {
            ...
            User.find({
                ...
                if (err) {
                    ...
                    newUser.usn = usn;
                    if (name) {
                        newUser.name.firstName = name.split(' ')[0];
                        newUser.name.lastName = name.split(' ')[1];
                    }
                    if (email) {
